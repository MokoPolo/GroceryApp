  ...
  (function (window, document, navigator, jQuery, undefined) {
    ...
    // Support three module loading scenarios
      if (typeof require === 'function' && typeof exports === 'object' && typeof module === 'object') {
        // [1] CommonJS/Node.js
        const target = module.exports || exports; // module.exports is for Node.js
        factory(target);
...
// In the future, the following "ko" variable may be made distinct from "koExports" so that private objects are not externally reachable.
      const ko = typeof koExports !== 'undefined' ? koExports : {};
// Google Closure Compiler helpers (used only to make the minified file smaller)
      ko.exportSymbol = function (koPath, object) {
        const tokens = koPath.split('.');
	...
	// At that point, "target" would be set to: (typeof koExports !== "undefined" ? koExports : ko)
      ...
      ko.utils = new (function () {
        const stringTrimRegex = /^(\s|\u00A0)+|(\s|\u00A0)+$/g;

    // Represent the known event types in a compact way, then at runtime transform it into a hash with event name as key (for fast lookup)
        let knownEvents = {},
          knownEventTypesByEventName = {};
        const keyEventTypeName = /Firefox\/2/i.test(navigator.userAgent) ? 'KeyboardEvent' : 'UIEvents';
        knownEvents[keyEventTypeName] = ['keyup', 'keydown', 'keypress'];
        knownEvents.MouseEvents = ['click', 'dblclick', 'mousedown', 'mouseup', 'mousemove', 'mouseover', 'mouseout', 'mouseenter', 'mouseleave'];
        for (const eventType in knownEvents) {
          const knownEventsForType = knownEvents[eventType];
          if (knownEventsForType.length) {
            for (let i = 0, j = knownEventsForType.length; i < j; i++)              { knownEventTypesByEventName[knownEventsForType[i]] = eventType; }
          }
        }
        const eventsThatMustBeRegisteredUsingAttachEvent = { propertychange: true }; // Workaround for an IE9 issue - https://github.com/SteveSanderson/knockout/issues/406
    ...
    // If there is a future need to detect specific versions of IE10+, we will amend this.
        ...
        return {
          ...
          arrayRemoveItem(array, itemToRemove) {
            const index = ko.utils.arrayIndexOf(array, itemToRemove);
            if (index >= 0)
          ...
          moveCleanedNodesToContainerElement(nodes) {
            ...
            const nodesArray = ko.utils.makeArray(nodes);
          ...
          replaceDomNodes(nodeToReplaceOrNodeArray, newNodesArray) {
            const nodesToReplaceArray = nodeToReplaceOrNodeArray.nodeType ? [nodeToReplaceOrNodeArray] : nodeToReplaceOrNodeArray;
          ...
          stringTokenize(string, delimiter) {
            const result = [];
            const tokens = (string || '').split(delimiter);
            for (let i = 0, j = tokens.length; i < j; i++) {
              let trimmed = ko.utils.stringTrim(tokens[i]);
              if (trimmed !== '')
                result.push(trimmed);
            }
            return result;
          ...
          registerEventHandler(element, eventType, handler) {
            ...
            if (!mustUseAttachEvent && typeof jQuery !== 'undefined') {
              ...
              element.attachEvent(`on${eventType}`, (event) => {
          ...
          toggleDomNodeCssClass(node, classNames, shouldHaveClass) {
            if (classNames) {
              let cssClassNameRegex = /[\w-]+/g,
                currentClassNames = node.className.match(cssClassNameRegex) || [];
              ko.utils.arrayForEach(classNames.match(cssClassNameRegex), (className) => {
          ...
          setTextContent(element, textContent) {
            let value = ko.utils.unwrapObservable(textContent);
            ...
            if (element.nodeType === 3) {
              ...
              let innerTextNode = ko.virtualElements.firstChild(element);
          ...
          getFormFields(form, fieldName) {
            const fields = ko.utils.makeArray(form.getElementsByTagName('input')).concat(ko.utils.makeArray(form.getElementsByTagName('textarea')));
            const isMatchingField = (typeof fieldName === 'string')
          ...
          stringifyJson(data, replacer, space) {   // replacer and space are optional
          ...
          postJson(urlOrForm, data, options) {
            ...
            const includeFields = options.includeFields || this.fieldsIncludedWithJsonPost;
            let url = urlOrForm;
    ...
    // In case the browser doesn't implement it natively, provide a JavaScript implementation. This implementation is based on the one in prototype.js
      ...
      ko.utils.domData = new (function () {
        let uniqueId = 0;
        const dataStoreKeyExpandoPropertyName = `__ko__${(new Date()).getTime()}`;
        const dataStore = {};
        return {
          get(node, key) {
            const allDataForNode = ko.utils.domData.getAll(node, false);
          ...
          set(node, key, value) {
            ...
            const allDataForNode = ko.utils.domData.getAll(node, true);
            allDataForNode[key] = value;
          },
          getAll(node, createIfNotFound) {
      ...
      ko.utils.domNodeDisposal = new (function () {
        const domDataKey = `__ko_domNodeDisposal__${(new Date()).getTime()}`;
        const cleanableNodeTypes = { 1: true, 8: true, 9: true };       // Element, Comment, Document
        const cleanableNodeTypesWithDescendants = { 1: true, 9: true }; // Element, Document

        function getDisposeCallbacksCollection(node, createIfNotFound) {
          let allDisposeCallbacks = ko.utils.domData.get(node, domDataKey);
      ...
      (function () {
        ...
        // Trim whitespace, otherwise indexOf won't work as expected
          let tags = ko.utils.stringTrim(html).toLowerCase(),
            div = document.createElement('div');

        // Finds the first match from the left column, and returns the corresponding "wrap" data from the right column
          const wrap = tags.match(/^<(thead|tbody|tfoot)/) && [1, '<table>', '</table>'] ||
                   !tags.indexOf('<tr') && [2, '<table><tbody>', '</tbody></table>'] ||
                   (!tags.indexOf('<td') || !tags.indexOf('<th')) && [3, '<table><tbody><tr>', '</tr></tbody></table>'] || [0, '', ''];
        ...
        //     so the target cannot change value synchronously or faster than a certain rate
          ...
          return ko.dependentObservable({
            ...
            write (value) {
              ...
              writeTimeoutInstance = setTimeout(() => {
              target(value);
            }, timeout);
            },
          });
      ...
      ko.subscribable.fn = {
        subscribe(callback, callbackTarget, event) {
          ...
          var subscription = new ko.subscription(this, boundCallback, () => {
        ...
        notifySubscribers(valueToNotify, event) {
          ...
          if (this._subscriptions[event]) {
            ko.dependencyDetection.ignore(function () {
              ko.utils.arrayForEach(this._subscriptions[event].slice(0), (subscription) => {
      ...
      ko.dependencyDetection = (function () {
        ...
        return {
          begin(callback) {
            _frames.push({ callback, distinctDependencies: [] });
          },
          ...
          registerDependency(subscribable) {
            ...
            if (_frames.length > 0) {
              let topFrame = _frames[_frames.length - 1];
      ...
      const primitiveTypes = { undefined: true, boolean: true, number: true, string: true };

      ko.observable = function (initialValue) {
        let _latestValue = initialValue;
      ...
      const protoProperty = ko.observable.protoProperty = '__ko_proto__';
    ...
    // Anything else
      ...
      ko.observableArray = function (initialValues) {
        ...
        const result = ko.observable(initialValues);
      ...
      ko.observableArray.fn = {
        remove(valueOrPredicate) {
          ...
          const removedValues = [];
          const predicate = typeof valueOrPredicate === 'function' ? valueOrPredicate : function (value) { return value === valueOrPredicate; };
        ...
        // If you passed an arg, we interpret it as an array of entries to remove
          ...
          return this.remove(value => ko.utils.arrayIndexOf(arrayOfValues, value) >= 0);
        },
        ...
        // If you passed zero args, we destroy everything
          if (arrayOfValues === undefined)            { return this.destroy(() => true); }

        // If you passed an arg, we interpret it as an array of entries to destroy
          if (!arrayOfValues)            { return []; }
          return this.destroy(value => ko.utils.arrayIndexOf(arrayOfValues, value) >= 0);
        },
...
// because we'll eval them without causing subscriptions, so ko.computed output could end up getting stale
      ko.utils.arrayForEach(['pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift'], (methodName) => {
        ...
        // (for consistency with mutating regular observables)
          const underlyingArray = this.peek();
          this.valueWillMutate();
          const methodCallResult = underlyingArray[methodName](...arguments);
          this.valueHasMutated();
...
// Populate ko.observableArray.fn with read-only functions from native arrays
      ko.utils.arrayForEach(['slice'], (methodName) => {
        ko.observableArray.fn[methodName] = function () {
          const underlyingArray = this();
          return underlyingArray[methodName](...arguments);
        };
      });
      ...
      ko.dependentObservable = function (evaluatorFunctionOrOptions, evaluatorFunctionTarget, options) {
        ...
        function disposeAllSubscriptionsToDependencies() {
          ko.utils.arrayForEach(_subscriptionsToDependencies, (subscription) => {
        ...
        function evaluatePossiblyAsync() {
          const throttleEvaluationTimeout = dependentObservable.throttleEvaluation;
        ...
        // going to be in the doc until *after* the first evaluation
          ...
          try {
            ...
            const disposalCandidates = ko.utils.arrayMap(_subscriptionsToDependencies, item => item.target);

            ko.dependencyDetection.begin((subscribable) => {
              let inOld;
            ...
            const newValue = readFunction.call(evaluatorFunctionTarget);

            // For each subscription no longer being used, remove it from the active subscriptions list and dispose it
            for (let i = disposalCandidates.length - 1; i >= 0; i--) {
    ...
    // plus adds a "disposeWhen" callback that, on each evaluation, disposes if the node was removed by some other means.)
        if (disposeWhenNodeIsRemoved && isActive()) {
          ...
          const existingDisposeWhenFunction = disposeWhen;
      ...
      const protoProp = ko.observable.protoProperty; // == "__ko_proto__"
      ...
      (function () {
        const maxNestedObservableDepth = 10; // Escape the (unlikely) pathalogical case where an observable's current value is itself (or similar reference cycle)

        ko.toJS = function (rootObject) {
          if (arguments.length == 0) { throw new Error('When calling ko.toJS, pass the object you want to convert.'); }

        // We just unwrap everything at every level in the object graph
          return mapJsObjectGraph(rootObject, (valueToMap) => {
            // Loop because an observable's value might in turn be another observable wrapper
            for (let i = 0; ko.isObservable(valueToMap) && (i < maxNestedObservableDepth); i++)              { valueToMap = valueToMap(); }
            return valueToMap;
          });
        };

        ko.toJSON = function (rootObject, replacer, space) {     // replacer and space are optional
          const plainJavaScriptObject = ko.toJS(rootObject);
        ...
        function mapJsObjectGraph(rootObject, mapInputCallback, visitedObjects) {
          ...
          const canHaveProperties = (typeof rootObject === 'object') && (rootObject !== null) && (rootObject !== undefined) && (!(rootObject instanceof Date));
          if (!canHaveProperties) { return rootObject; }

          const outputProperties = rootObject instanceof Array ? [] : {};
          visitedObjects.save(rootObject, outputProperties);

          visitPropertiesOrArrayEntries(rootObject, (indexer) => {
        ...
        function visitPropertiesOrArrayEntries(rootObject, visitorCallback) {
          if (rootObject instanceof Array) {
            for (let i = 0; i < rootObject.length; i++)              { visitorCallback(i); }

            // For arrays, also respect toJSON property for custom mappings (fixes #278)
            if (typeof rootObject.toJSON === 'function')              { visitorCallback('toJSON'); }
          } else {
            for (const propertyName in rootObject)              { visitorCallback(propertyName); }
          }
        }
    ...
    // that are arbitrary objects. This is very convenient when implementing things like cascading dropdowns.
      ...
      ko.expressionRewriting = (function () {
        const restoreCapturedTokensRegex = /\@ko_token_(\d+)\@/g;
        const javaScriptReservedWords = ['true', 'false'];
    ...
    // This is designed to be simple and avoid false negatives, but could produce false positives (e.g., a+b.c).
        ...
        function restoreTokens(string, tokens) {
          ...
          while (string != prevValue) { // Keep restoring tokens until it no longer makes a difference (they may be nested)
            prevValue = string;
            string = string.replace(restoreCapturedTokensRegex, (match, tokenIndex) => tokens[tokenIndex]);
          }
          return string;
        }
        ...
        function ensureQuoted(key) {
          const trimmedKey = ko.utils.stringTrim(key);
        ...
        return {
          ...
          parseObjectLiteral(objectLiteralString) {
            ...
            let str = ko.utils.stringTrim(objectLiteralString);
            if (str.length < 3)
              {return [];}
            ...
            for (let i = 0, j = keyValuePairs.length; i < j; i++) {
              let pair = keyValuePairs[i];
              let colonPos = pair.indexOf(':');
          ...
          preProcessBindings(objectLiteralStringOrKeyValueArray) {
            const keyValueArray = typeof objectLiteralStringOrKeyValueArray === 'string'
            ...
            let keyValueEntry;
            for (let i = 0; keyValueEntry = keyValueArray[i]; i++) {
            ...
            let combinedResult = resultStrings.join('');
            if (propertyAccessorResultStrings.length > 0) {
              let allPropertyAccessors = propertyAccessorResultStrings.join('');
        ...
        //                      it is !== existing value on that writable observable
          writeValueToProperty(property, allBindingsAccessor, key, value, checkIfDifferent) {
...
// they referred to JSON specifically, even though they actually work with arbitrary JavaScript object literal expressions.)
    ...
    // So, use node.text where available, and node.nodeValue elsewhere
        const commentNodesHaveTextProperty = document.createComment('test').text === '<!--test-->';

        const startCommentRegex = commentNodesHaveTextProperty ? /^<!--\s*ko(?:\s+(.+\s*\:[\s\S]*))?\s*-->$/ : /^\s*ko(?:\s+(.+\s*\:[\s\S]*))?\s*$/;
        const endCommentRegex = commentNodesHaveTextProperty ? /^<!--\s*\/ko\s*-->$/ : /^\s*\/ko\s*$/;
        ...
        ko.virtualElements = {
          ...
          emptyNode(node) {
            if (!isStartComment(node))
              ...
              let virtualChildren = ko.virtualElements.childNodes(node);
          ...
          virtualNodeBindingValue(node) {
            const regexMatch = isStartComment(node);
            return regexMatch ? regexMatch[1] : null;
          },

          normaliseVirtualElementDomStructure(elementVerified) {
...
// ko.exportSymbol('virtualElements.nextSibling', ko.virtualElements.nextSibling);   // nextSibling is not minified
      ...
      (function () {
        ...
        ko.utils.extend(ko.bindingProvider.prototype, {
          ...
          getBindings(node, bindingContext) {
            const bindingsString = this.getBindingsString(node, bindingContext);
        ...
        // It's not part of the interface definition for a general binding provider.
          parseBindingsString(bindingsString, bindingContext, node) {
            try {
              let bindingFunction = createBindingsStringEvaluatorViaCache(bindingsString, this.bindingCache);
        ...
        // Example result: with(sc1) { with(sc0) { return (expression) } }
          let rewrittenBindings = ko.expressionRewriting.preProcessBindings(bindingsString),
      ...
      (function () {
        ...
        ko.bindingContext.prototype.extend = function (properties) {
          const clone = ko.utils.extend(new ko.bindingContext(), this);
          return ko.utils.extend(clone, properties);
        };

        function validateThatBindingIsAllowedForVirtualElements(bindingName) {
          const validator = ko.virtualElements.allowedBindings[bindingName];
        ...
        function applyBindingsToNodeAndDescendantsInternal(viewModel, nodeVerified, bindingContextMayDifferFromDomParentElement) {
          let shouldBindDescendants = true;
        ...
        // (2) It might have bindings (e.g., it has a data-bind attribute, or it's a marker for a containerless template)
          ...
          const shouldApplyBindings = (isElement && bindingContextMayDifferFromDomParentElement)             // Case (1)
        ...
        // Need to be sure that inits are only run once, and updates never run until all the inits have been run
          let initPhase = 0; // 0 = before all inits, 1 = during inits, 2 = after all inits
        ...
        // associated with this node's bindings) that all the closures can access.
          ...
          ko.dependentObservable(
            () => {
              ...
              const bindingContextInstance = viewModelOrBindingContext && (viewModelOrBindingContext instanceof ko.bindingContext)
                    ? viewModelOrBindingContext
                    : new ko.bindingContext(ko.utils.unwrapObservable(viewModelOrBindingContext));
              const viewModel = bindingContextInstance.$data;
              ...
              const evaluatedBindings = (typeof bindings === 'function') ? bindings(bindingContextInstance, node) : bindings;
                ...
                if (initPhase === 0) {
                  ...
                  for (var bindingKey in parsedBindings) {
                    ...
                    if (binding && typeof binding.init === 'function') {
                      const handlerInitFn = binding.init;
                      const initResult = handlerInitFn(node, makeValueAccessor(bindingKey), parsedBindingsAccessor, viewModel, bindingContextInstance);
                ...
                if (initPhase === 2) {
                  for (var bindingKey in parsedBindings) {
                    ...
                    if (binding && typeof binding.update === 'function') {
                      const handlerUpdateFn = binding.update;
        ...
        const storedBindingContextDomDataKey = '__ko_bindingContext__';
        ko.storedBindingContextForNode = function (node, bindingContext) {
          if (arguments.length == 2) { ko.utils.domData.set(node, storedBindingContextDomDataKey, bindingContext); } else { return ko.utils.domData.get(node, storedBindingContextDomDataKey); }
        };
    ...
    // Retrieving binding context from arbitrary nodes
      ...
      const attrHtmlToJavascriptMap = { class: 'className', for: 'htmlFor' };
      ko.bindingHandlers.attr = {
        update(element, valueAccessor, allBindingsAccessor) {
          const value = ko.utils.unwrapObservable(valueAccessor()) || {};
          for (let attrName in value) {
            if (typeof attrName === 'string') {
              let attrValue = ko.utils.unwrapObservable(value[attrName]);
      ...
      ko.bindingHandlers.checked = {
        init(element, valueAccessor, allBindingsAccessor) {
          const updateHandler = function () {
            ...
            if ((element.type == 'checkbox') && (unwrappedValue instanceof Array)) {
              ...
              let existingEntryIndex = ko.utils.arrayIndexOf(unwrappedValue, element.value);
        ...
        // IE 6 won't allow radio buttons to be selected unless they have a name
          if ((element.type == 'radio') && !element.name)            { ko.bindingHandlers.uniqueName.init(element, () => true); }
        },
      ...
      const classesWrittenByBindingKey = '__ko__cssValue';
      ko.bindingHandlers.css = {
        update(element, valueAccessor) {
          let value = ko.utils.unwrapObservable(valueAccessor());
          if (typeof value === 'object') {
            for (const className in value) {
              let shouldHaveClass = ko.utils.unwrapObservable(value[className]);
      ...
      ko.bindingHandlers.event = {
        init(element, valueAccessor, allBindingsAccessor, viewModel) {
          ...
          for (var eventNameOutsideClosure in eventsToHandle) {
            (function () {
              let eventName = eventNameOutsideClosure; // Separate variable to be captured by event handler closure
...
// "foreach: { data: someExpression, afterAdd: myfn }" is equivalent to "template: { foreach: someExpression, afterAdd: myfn }"
      ...
      const withIfDomDataKey = '__ko_withIfBindingData';
// Makes a binding like with or if
      function makeWithIfBinding(bindingKey, isWith, isNot, makeContextCallback) {
        ko.bindingHandlers[bindingKey] = {
          ...
          update(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            let withIfData = ko.utils.domData.get(element, withIfDomDataKey),
...
// Construct the actual binding handlers
    ...
    (bindingContext, dataValue) => bindingContext.createChildContext(dataValue),
);
    ...
    // change the model value to match the dropdown.
      ...
      ko.bindingHandlers.options = {
        update(element, valueAccessor, allBindingsAccessor) {
          ...
          const selectWasPreviouslyEmpty = element.length == 0;
          const previousSelectedValues = ko.utils.arrayMap(ko.utils.arrayFilter(element.childNodes, node => node.tagName && (ko.utils.tagNameLower(node) === 'option') && node.selected), node => ko.selectExtensions.readValue(node) || node.innerText || node.textContent);
        ...
        // Need to use .remove() rather than .removeChild() for <option>s otherwise IE behaves oddly (https://github.com/SteveSanderson/knockout/issues/134)
          ...
          if (value) {
            ...
            for (var i = 0, j = value.length; i < j; i++) {
              ...
              const optionValue = applyToObject(arrayEntry, allBindings.optionsValue, arrayEntry);
              ko.selectExtensions.writeValue(option, ko.utils.unwrapObservable(optionValue));

                // Apply some text to the option element
              const optionText = applyToObject(arrayEntry, allBindings.optionsText, optionValue);
            ...
            const newOptions = element.getElementsByTagName('option');
            let countSelectionsRetained = 0;
      ...
      ko.bindingHandlers.selectedOptions = {
        init(element, valueAccessor, allBindingsAccessor) {
          ko.utils.registerEventHandler(element, 'change', () => {
            let value = valueAccessor(),
              valueToWrite = [];
            ko.utils.arrayForEach(element.getElementsByTagName('option'), (node) => {
              if (node.selected)
        ...
        update(element, valueAccessor) {
          ...
          if (newValue && typeof newValue.length === 'number') {
            ko.utils.arrayForEach(element.getElementsByTagName('option'), (node) => {
              let isSelected = ko.utils.arrayIndexOf(newValue, ko.selectExtensions.readValue(node)) >= 0;
        ...
        update(element, valueAccessor) {
          ...
          for (const styleName in value) {
            if (typeof styleName === 'string') {
              let styleValue = ko.utils.unwrapObservable(value[styleName]);
      ...
      ko.bindingHandlers.submit = {
        init(element, valueAccessor, allBindingsAccessor, viewModel) {
          ...
          ko.utils.registerEventHandler(element, 'submit', (event) => {
            ...
            try { handlerReturnValue = value.call(viewModel, element); } finally {
              if (handlerReturnValue !== true) { // Normally we want to prevent default action. Developer can override this be explicitly returning true.
                if (event.preventDefault)
                event.preventDefault();
                else { event.returnValue = false; }
      ...
      ko.bindingHandlers.uniqueName = {
        init(element, valueAccessor) {
          if (valueAccessor()) {
            const name = `ko_unique_${++ko.bindingHandlers.uniqueName.currentIndex}`;
      ...
      ko.bindingHandlers.value = {
        ...
        // Always catch "change" event; possibly other events too if asked
          ...
          let requestedEventsToCatch = allBindingsAccessor().valueUpdate;
          ...
          const valueUpdateHandler = function () {
        ...
        // IE doesn't fire "change" events on textboxes if the user selects a value from its autocomplete list
          const ieAutoCompleteHackNeeded = ko.utils.ieVersion && element.tagName.toLowerCase() == 'input' && element.type == 'text'
        ...
        update(element, valueAccessor) {
          const valueIsSelectOption = ko.utils.tagNameLower(element) === 'select';
          const newValue = ko.utils.unwrapObservable(valueAccessor());
          const elementValue = ko.selectExtensions.readValue(element);
          let valueHasChanged = (newValue != elementValue);
        ...
        // We don't want to do a strict equality comparison as that is more confusing for developers in certain cases, so we specifically special case 0 != "" here.
          ...
          if (valueHasChanged) {
            const applyValueAction = function () { ko.selectExtensions.writeValue(element, newValue); };
            applyValueAction();

            // Workaround for IE6 bug: It won't reliably apply values to SELECT nodes during the same execution thread
            // right after you've changed the set of OPTION nodes on it. So for that node type, we'll schedule a second thread
            // to apply the value as well.
            const alsoApplyAsynchronously = valueIsSelectOption;
...
//     and then you don't need to override 'createJavaScriptEvaluatorBlock'.
    ...
    // Skip rewriting if requested
      ...
      ko.templateRewriting = (function () {
        const memoizeDataBindingAttributeSyntaxRegex = /(<[a-z]+\d*(\s+(?!data-bind=)[a-z0-9\-]+(=(\"[^\"]*\"|\'[^\']*\'))?)*\s+)data-bind=(["'])([\s\S]*?)\5/gi;
        const memoizeVirtualContainerBindingSyntaxRegex = /<!--\s*ko\b\s*([\s\S]*?)\s*-->/g;

        function validateDataBindValuesForRewriting(keyValueArray) {
          const allValidators = ko.expressionRewriting.bindingRewriteValidators;
          for (let i = 0; i < keyValueArray.length; i++) {
            const key = keyValueArray[i].key;
            if (allValidators.hasOwnProperty(key)) {
              const validator = allValidators[key];

              if (typeof validator === 'function') {
                let possibleErrorMessage = validator(keyValueArray[i].value);
        ...
        function constructMemoizedTagReplacement(dataBindAttributeValue, tagToRetain, templateEngine) {
          const dataBindKeyValueArray = ko.expressionRewriting.parseObjectLiteral(dataBindAttributeValue);
          validateDataBindValuesForRewriting(dataBindKeyValueArray);
          const rewrittenDataBindAttributeValue = ko.expressionRewriting.preProcessBindings(dataBindKeyValueArray);
        ...
        return {
          ensureTemplateIsRewritten(template, templateEngine, templateDocument) {
            if (!templateEngine.isTemplateRewritten(template, templateDocument))
              {templateEngine.rewriteTemplate(template, htmlString => ko.templateRewriting.memoizeBindingAttributeSyntax(htmlString, templateEngine), templateDocument);}
          },
...
// Exported only because it has to be referenced by string lookup from within rewritten template
    ...
    // ---- ko.templateSources.domElement -----
        ...
        ko.templateSources.domElement.prototype.text = function (/* valueToWrite */) {
          let tagNameLower = ko.utils.tagNameLower(this.domElement),
    ...
    // Writing to "text" is still supported, but then the template data will not be available as DOM nodes.
        ...
        const anonymousTemplatesDomDataKey = '__ko_anon_template__';
        ...
        ko.templateSources.anonymousTemplate.prototype.text = function (/* valueToWrite */) {
          if (arguments.length == 0) {
            const templateData = ko.utils.domData.get(this.domElement, anonymousTemplatesDomDataKey) || {};
      ...
      (function () {
        ...
        // (2) Unmemoizes any memos in the DOM subtree (e.g., to activate bindings that had been memoized during template rewriting)
          ...
          if (continuousNodeArray.length) {
            ...
            invokeForEachNodeOrCommentInContinuousRange(firstNode, lastNode, (node) => {
        ...
        function executeTemplate(targetNodeOrNodeArray, renderMode, template, bindingContext, options) {
          ...
          const templateDocument = firstTargetNode && firstTargetNode.ownerDocument;
        ...
        ko.renderTemplate = function (template, dataOrBindingContext, options, targetNodeOrNodeArray, renderMode) {
          ...
          if (targetNodeOrNodeArray) {
            ...
            const whenToDispose = function () { return (!firstTargetNode) || !ko.utils.domNodeIsAttachedToDocument(firstTargetNode); }; // Passive disposal (on next evaluation)
            ...
            return ko.dependentObservable( // So the DOM is automatically updated when any dependency changes
                () => {
                  ...
                  const bindingContext = (dataOrBindingContext && (dataOrBindingContext instanceof ko.bindingContext))
                        ? dataOrBindingContext
                        : new ko.bindingContext(ko.utils.unwrapObservable(dataOrBindingContext));

                    // Support selecting template as a function of the data being rendered
                  const templateName = typeof (template) === 'function' ? template(bindingContext.$data, bindingContext) : template;

                  const renderedNodesArray = executeTemplate(targetNodeOrNodeArray, renderMode, templateName, bindingContext, options);
        ...
        // This will be called by setDomNodeChildrenFromArrayMapping to get the nodes to add to targetNode
          const executeTemplateForArrayItem = function (arrayValue, index) {
            // Support selecting template as a function of the data being rendered
            arrayItemContext = parentBindingContext.createChildContext(ko.utils.unwrapObservable(arrayValue), options.as);
            arrayItemContext.$index = index;
            const templateName = typeof (template) === 'function' ? template(arrayValue, arrayItemContext) : template;
        ...
        // This will be called whenever setDomNodeChildrenFromArrayMapping has added nodes to targetNode
          ...
          return ko.dependentObservable(() => {
            let unwrappedArray = ko.utils.unwrapObservable(arrayOrObservableArray) || [];
            if (typeof unwrappedArray.length === 'undefined') // Coerce single value into array
              { unwrappedArray = [unwrappedArray]; }

            // Filter out any entries marked as destroyed
            const filteredArray = ko.utils.arrayFilter(unwrappedArray, item => options.includeDestroyed || item === undefined || item === null || !ko.utils.unwrapObservable(item._destroy));
        ...
        const templateComputedDomDataKey = '__ko__templateComputedDomDataKey__';
        function disposeOldComputedAndStoreNewOne(element, newComputed) {
          const oldComputed = ko.utils.domData.get(element, templateComputedDomDataKey);
        ...
        ko.bindingHandlers.template = {
          init(element, valueAccessor) {
            ...
            const bindingValue = ko.utils.unwrapObservable(valueAccessor());
            if ((typeof bindingValue !== 'string') && (!bindingValue.name) && (element.nodeType == 1 || element.nodeType == 8)) {
                // It's an anonymous template - store the element contents, then clear the element
              let templateNodes = element.nodeType == 1 ? element.childNodes : ko.virtualElements.childNodes(element),
          ...
          update(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            let templateName = ko.utils.unwrapObservable(valueAccessor()),
              options = {},
              shouldDisplay = true,
              dataValue,
    ...
    // Anonymous templates can't be rewritten. Give a nice error message if you try to do it.
        ko.expressionRewriting.bindingRewriteValidators.template = function (bindingValue) {
          const parsedBindingValue = ko.expressionRewriting.parseObjectLiteral(bindingValue);
    ...
    // Simple calculation based on Levenshtein distance.
        ...
        function compareSmallArrayToBigArray(smlArray, bigArray, statusNotInSml, statusNotInBig, dontLimitMoves) {
          ...
          if (notInSml.length && notInBig.length) {
            ...
            let limitFailedCompares = smlIndexMax * 10,
              failedCompares,
              a,
              d,
              notInSmlItem,
    ...
    // You can use this, for example, to activate bindings on those nodes.
        ...
        // Rule [B]
          if (contiguousNodeArray.length > 1) {
            ...
            let current = contiguousNodeArray[0],
        ...
        // Map this array value inside a dependentObservable so we re-map when any dependency changes
          const mappedNodes = [];
          const dependentObservable = ko.dependentObservable(() => {
            const newMappedNodes = mapping(valueToMap, index) || [];
          ...
          }, null, { disposeWhenNodeIsRemoved: containerNode, disposeWhen() { return (mappedNodes.length == 0) || !ko.utils.domNodeIsAttachedToDocument(mappedNodes[0]); } });
          return { mappedNodes, dependentObservable: (dependentObservable.isActive() ? dependentObservable : undefined) };
        }

        const lastMappingResultDomDataKey = 'setDomNodeChildrenFromArrayMapping_lastMappingResult';

        ko.utils.setDomNodeChildrenFromArrayMapping = function (domNode, array, mapping, options, callbackAfterAddingNodes) {
        // Compare the provided array against the previous one
          array = array || [];
          options = options || {};
          const isFirstExecution = ko.utils.domData.get(domNode, lastMappingResultDomDataKey) === undefined;
          const lastMappingResult = ko.utils.domData.get(domNode, lastMappingResultDomDataKey) || [];
          const lastArray = ko.utils.arrayMap(lastMappingResult, x => x.arrayEntry);
          const editScript = ko.utils.compareArrays(lastArray, array);

        // Build the new mapping result
          const newMappingResult = [];
          ...
          function callCallback(callback, items) {
            if (callback) {
              for (var i = 0, n = items.length; i < n; i++) {
                if (items[i]) {
                  ko.utils.arrayForEach(items[i].mappedNodes, (node) => {
            ...
            switch (editScriptItem.status) {
              case 'deleted':
                if (movedIndex === undefined) {
                  ...
                  nodesToDelete.push(...fixUpNodesToBeMovedOrRemoved(mapData.mappedNodes));
      ...
      ko.nativeTemplateEngine.prototype.renderTemplateSource = function (templateSource, bindingContext, options) {
        let useNodesIfAvailable = !(ko.utils.ieVersion < 9), // IE<9 cloneNode doesn't work properly
      ...
      (function () {
        ...
        // which KO internally refers to as version "2", so older versions are no longer detected.
          const jQueryTmplVersion = this.jQueryTmplVersion = (function () {
          ...
          this.renderTemplateSource = function (templateSource, bindingContext, options) {
            ...
            const data = [bindingContext.$data]; // Prewrap the data in an array to stop jquery.tmpl from trying to unwrap any arrays
            const jQueryTemplateOptions = jQuery.extend({ koBindingContext: bindingContext }, options.templateOptions);

            const resultNodes = executeTemplate(precompiled, data, jQueryTemplateOptions);
    ...
    // Use this one by default *only if jquery.tmpl is referenced*
        const jqueryTmplTemplateEngineInstance = new ko.jqueryTmplTemplateEngine();
